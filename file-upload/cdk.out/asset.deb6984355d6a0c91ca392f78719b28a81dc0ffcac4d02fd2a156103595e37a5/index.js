...
async function main(event) {
  ...
  try {
    if (!((_a = event.queryStringParameters) == null ? void 0 : _a.fileType) || !((_b = event.queryStringParameters) == null ? void 0 : _b.fileTopic)) {
    ...
    return {
      ...
      body: JSON.stringify(__spreadProps(__spreadValues({}, presignedURL), {
        filePath
      }))
    };
  } catch (error) {
...
function createPresignedURL({ fileType, fileTopic, fileName }) {
  const params = {
    Bucket: process.env.BUCKET_NAME,
    Fields: { key: `${fileTopic}/${fileName}`, acl: "public-read" },
    Conditions: [
      ["content-length-range", 0, 1e7],
